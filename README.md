...
- an odds ratio (association parameter) $$\phi$$,

we solve for the joint cell probabilities
...
and the odds ratio constraint

$$
\text{OR} = \frac{p_{11} p_{00}}{p_{10} p_{01}} = \phi.
$$

This formulation typically yields a quadratic equation in $$p_{11}$$.  
The valid solution is the root that lies within the feasible range:

$$
\max(0, p_E + p_S - 1) \le p_{11} \le \min(p_E, p_S).
$$

When $$\phi = 1$$, the two endpoints are independent and
...
**Multinomial** distribution with four categories (a $$2 \times 2$$ table).

For a sample size $$n$$, the count vector
...
$$
(N_{11}, N_{10}, N_{01}, N_{00})
\sim \text{Multinomial}\bigl(n;\; p_{11}, p_{10}, p_{01}, p_{00}\bigr).
$$
